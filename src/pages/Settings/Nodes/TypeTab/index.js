...
let index = 0;
function createData(NodeName, Status, editBtn) {
  index += 1;
  return {index, NodeName, Status, editBtn};
}
...
class EnhancedUserTable extends React.Component {
  ...
  handleSelectAllClick = (event, checked) => {
    ...
    if (checked) {
      this.setState({
        selected: data.length > 0 && data.map((n) => n.node_name),
      });
      return;
    }
  ...
  render() {
    ...
    return (
      <div className={classes.root}>
        <Grid md={12} item>
          {location.pathname !== '/settings/nodes/new-node-type' ? (
            <div>
